...
**「怎麼跑」由 adapter 實作**：`adapters/windows/*.ps1` 是 **.NET/Windows 的參考 adapter**（PowerShell 是該生態系的原生工具）——它是**一個 adapter，不是通用機制**。其他生態系用各自的 adapter；無對應 adapter 時，agent 直接照契約用該環境的工具執行。
...
groundwork/                          ← plugin root
  ...
  adapters/
    windows/  {verify.ps1, collect.ps1, feedback.ps1}  ← 平台=目錄, 工具=檔名（.NET/Windows 參考實作）
...
## windows adapter 用法（實作 contract 的參考腳本）
...
- windows adapter 對真實專案實測通過；硬化項（WER 誤判、harness hash、limitations JSON、最小 metrics、feedback redaction）已修並驗證。
